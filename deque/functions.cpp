...
void readFile (std::deque<Student> &S, std::string fileName) {
    ...
    while (std::getline(fd, row)) {         // Continue reading until the end of file is reached (raised error flag)
        ...
        temp.HW.clear();                    // Empty the temporary homework deque and fill it with grades from the file
...
double finalGrade (Student* S, char type){
    double hw;
    if (!S->HW.empty())  // Calculate the homework grade only if the homework deque isn't empty
        type == 'm' ? hw = median(&S->HW[0], S->HW.size()) : hw = average(&S->HW[0], S->HW.size());
    else hw = 0;
    return (0.4 * hw + 0.6 * S->exam);
}

void makeGroups (std::deque<Student> &S, std::deque<Student> &GS) {
    ...
    //GS.reserve(S.size() - numOfBadStudents);    // Increase required capacity for the "good students" deque
...
void sort (std::deque<Student> &S, std::deque<Student> &GS, char sortType) {
    if (sortType == 'v') {
...
void writeToFile(std::deque<Student> &S, char finalType, std::string fileName) {
